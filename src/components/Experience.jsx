import React from "react";
import html from "../assets/html.png";
import css from "../assets/css.png";
import javascript from "../assets/javascript.png";
import reactImage from "../assets/react.png";
import node from "../assets/node.png";
import database from "../assets/database.png";
import problemsolving from "../assets/problem-solving.png";
import github from "../assets/github.png";
import tailwind from "../assets/tailwind.png";
import python from "../assets/python.png";
import ai from "../assets/ai.png";
import deeplearning from "../assets/deep-learning (1).png";


const Experience = () => {
  const techs = [
    {
      id: 1,
      src: html,
      title: "HTML",
      style: "shadow-orange-500",
    },
    {
      id: 2,
      src: css,
      title: "CSS",
      style: "shadow-blue-300",
    },
    {
      id: 3,
      src: javascript,
      title: "JavaScript",
      style: "shadow-yellow-500",
    },
    {
      id: 4,
      src: reactImage,
      title: "ReactJS",
      style: "shadow-blue-700",
    },
    {
      id: 5,
      src: tailwind,
      title: "Tailwind",
      style: "shadow-sky-400",
    },
    {
      id: 6,
      src: database,
      title: "MongoDB",
      style: "shadow-yellow-200",
    },
    {
      id: 7,
      src: node,
      title: "NodeJS",
      style: "shadow-lime-400",
    },
    {
      id: 8,
      src: ai,
      title: "Machine Learning",
      style: "shadow-purple-600",
    },
    {
      id: 9,
      src: python,
      title: "Python",
      style: "shadow-indigo-400",
    },
    {
      id: 10,
      src: github,
      title: "GitHub",
      style: "shadow-gray-400",
    },
    {
      id: 11,
      src: problemsolving,
      title: "Competitive Programming with C++",
      style: "shadow-rose-300",
    },
    {
      id: 12,
      src: deeplearning,
      title: "Deep Learning",
      style: "shadow-pink-700",
    },
  ];

  return (
    <div
      name="experience"
      className="w-full bg-gradient-to-b from-black to-gray-800 p-4 text-white"
    >
      <div className="flex flex-col p-4 justify-center max-w-screen-lg mx-auto h-full min-h-screen">
        <div>
          <p className="text-4xl font-bold border-b-4 border-gray-500 pt-60 inline">
            Experience
          </p>
          <p className="py-6">I possess expertise in diverse technologies and domains through hands-on experience in my professional journey.</p>
        </div>

        <div className="w-full grid grid-cols-2 sm:grid-cols-3 gap-8 text-center py-8 px-12 sm:px-0">
          {techs.map(({ id, src, title, style }) => (
            <div
              key={id}
              className={`shadow-md hover:scale-105 duration-500 py-2 rounded-lg ${style}`}
            >
              <img src={src} alt="" className="w-20 mx-auto" />
              <p className="mt-4">{title}</p>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default Experience;
